extends Node
## HEADFUL acceptance guard for the screen BOUNDARY edge-drag end-to-end (ADR-0087, screen
## lane). Drives the REAL EffectViewer scene → EffectStudioPage edge-drag handlers → host
## studio_begin_coalesce / studio_apply_edit / studio_end_coalesce → EffectEditSession →
## ScreenChannel boundary trade, on real E015 (the screen-authoring pilot effect). Confirms
## the whole path an author's screen edge-drag drives:
##   * dragging an internal boundary is a stay-local sum-preserving trade — the dragged
##     span's end moves AND the array-adjacent neighbour's derived start follows, downstream
##     pinned,
##   * the boundary snaps to an 8-frame step,
##   * the effect re-folds (read-live screen) without dying,
...
## ScreenBoundaryEditTest / EffectStudioEdgeDragWiringTest); this is the live wiring on real
## bytes.
##
## Kept OUT of run_all_tests.sh (needs the gitignored E015 extract; acceptance precedent).
## Run: <GODOT> --path . --quit-after 200 res://tests/EffectScreenEdgeDragAcceptanceTest.tscn
...
func _ready() -> void:
	await _test_screen_edge_drag_on_real_E015()

	print("\n=== EffectScreenEdgeDragAcceptanceTest: %d passed, %d failed ===" % [_passed, _failed])
	if _failed > 0:
		print("[FAIL] EffectScreenEdgeDragAcceptanceTest")
		get_tree().quit(1)
	else:
...
func _test_screen_edge_drag_on_real_E015() -> void:
	...
	for d in page._effect_dirs:
		if String(d).ends_with("E015"):
			dir = d
	if dir == "":
		print("[SKIP] E015 extract absent — screen edge-drag acceptance skipped")
		return
	page._load_effect(dir)
	await _frames(20)

	# An internal screen boundary: a span whose next lane sibling is the NEXT array keyframe
	# (keyframe_index + 1) so the sum-preserving trade is exact and observable.
	var pick := _internal_screen_boundary(page)
	if pick.is_empty():
		print("[SKIP] no internal screen boundary on E015 — acceptance skipped")
		return
	...
	if target_local < 0:
		print("[SKIP] screen boundary has no 8-frame headroom on E015 — acceptance skipped")
		return
...
## The screen span with an array-adjacent next sibling (keyframe_index + 1) and the MOST
...
func _internal_screen_boundary(page) -> Dictionary:
	var best := {}
	var best_room := 0
