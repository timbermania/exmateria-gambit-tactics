...
## GPU Formula 32 Test -- MA * Y (no faith scaling)
##
## Kikuichimoji (ID 83): formula 32, Y=16, range 8, ct 0
## Caster: MA=10 → expected damage = 10 * 16 = 160

const ABILITY_ID = 83  # Kikuichimoji
const EXPECTED = 160
...
func get_test_name() -> String:
	return "GPU Formula 32 Test (MA*Y)"
...
func _ready():
	max_ticks = 2000
	super._ready()
	print("\n  Expected: MA(10) * Y(16) = %d" % EXPECTED)
...
func on_hp_changed(unit_idx: int, old_hp: int, new_hp: int, delta: int):
	...
	if unit_idx == 1 and delta < 0:
		...
		_done = true
		print("\n[%s] Formula 32 (MA*Y): expected=%d actual=%d  (HP: %d->%d)" % [
...
func on_victory(winning_team: int):
	if not _done:
		print("\n[FAIL] Formula 32: no damage observed before victory")
