...
## GPU Formula 49 Test -- ((PA + Y) / 2) * PA
##
## WaveFist (ID 102): formula 49, Y=2, range 3, ct 0
## Caster: PA=10 → expected = ((10 + 2) / 2) * 10 = 6 * 10 = 60

const ABILITY_ID = 102  # WaveFist
const EXPECTED = 60
...
func get_test_name() -> String:
	return "GPU Formula 49 Test (Hybrid PA)"
...
func _ready():
	max_ticks = 2000
	super._ready()
	print("\n  Expected: ((PA(10) + Y(2)) / 2) * PA(10) = %d" % EXPECTED)
...
func on_hp_changed(unit_idx: int, old_hp: int, new_hp: int, delta: int):
	...
	if unit_idx == 1 and delta < 0:
		...
		_done = true
		print("\n[%s] Formula 49 (Hybrid PA): expected=%d actual=%d  (HP: %d->%d)" % [
...
func on_victory(winning_team: int):
	if not _done:
		print("\n[FAIL] Formula 49: no damage observed before victory")
