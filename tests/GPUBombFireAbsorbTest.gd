...
## GPU Bomb-Fire Job-Innate Absorb Test (issue #111)
##
## Witnesses per-job innate element defense: a Bomb-jobbed target (job key
## "64") absorbs Fire from its job innate alone, with no Flame-Shield-style
## equipment carrying the absorb. Pairs with [code]GPUElementAbsorbTest[/code]
## (equipment-driven Fire absorb) — same shader path, the only thing under
## test is the job-innate OR landing in [code]element_absorb_mask[/code].
##
## Layout (Manhattan):
##   Mage (team 0, caster) at (0, 0)
##   Bomb (team 1, job 0x64) at (3, 0) — in Fire range 4
##
## PASS: Bomb HP rises above starting HP (Fire absorbed → heal).
## FAIL: HP stays flat or drops, or absorb mask missing Fire bit.

const ABILITY_FIRE = 16
const BOMB_JOB_ID = "64"
const TARGET_START_HP = 100
const TARGET_MAX_HP = 999  # well above start so the heal isn't capped


func get_test_name() -> String:
	return "GPU Bomb Fire Job-Innate Absorb Test"
...
func _process(delta: float) -> void:
	...
	if hp_now < TARGET_START_HP:
		# Damage actually landed — job-innate absorb didn't fire; capture and bail.
		_print_results(states)
		return
...
func _print_results(states: Array) -> void:
	...
	print("\n=== BOMB FIRE JOB-INNATE ABSORB TEST RESULTS ===")
	print("  Bomb HP: start=%d, now=%d (delta=%+d)" % [
	...
	var innate_ok := (absorb_mask & fire_bit_mask) != 0
	var hp_up := _absorb_witnessed_tick >= 0 and hp_now > TARGET_START_HP
	if innate_ok and hp_up:
		print("\n[PASS] Bomb absorbed Fire via job-innate (HP +%d)" % _absorb_hp_delta)
	else:
		var why: Array = []
		if not innate_ok: why.append("absorb mask missing Fire bit (job-innate OR failed in _build_gpu_config)")
