...
## GPU Formula 36 Test -- ((PA + Y) / 2) * MA
##
## Pitfall (ID 126): formula 36, Y=2, range 5, ct 0
## Caster: PA=10, MA=10 → expected = ((10 + 2) / 2) * 10 = 6 * 10 = 60

const ABILITY_ID = 126  # Pitfall
const EXPECTED = 60
...
func get_test_name() -> String:
	return "GPU Formula 36 Test (Hybrid MA)"
...
func _ready():
	max_ticks = 2000
	super._ready()
	print("\n  Expected: ((PA(10) + Y(2)) / 2) * MA(10) = %d" % EXPECTED)
...
func on_hp_changed(unit_idx: int, old_hp: int, new_hp: int, delta: int):
	...
	if unit_idx == 1 and delta < 0:
		...
		_done = true
		print("\n[%s] Formula 36 (Hybrid MA): expected=%d actual=%d  (HP: %d->%d)" % [
...
func on_victory(winning_team: int):
	if not _done:
		print("\n[FAIL] Formula 36: no damage observed before victory")
