...
## GPU Formula 55 Test -- PA * Y
##
## ThrowStone (ID 148): formula 55, Y=2, range 4, ct 0
## Caster: PA=10 → expected damage = 10 * 2 = 20

const ABILITY_ID = 148  # ThrowStone
const EXPECTED = 20
...
func get_test_name() -> String:
	return "GPU Formula 55 Test (PA*Y)"
...
func on_hp_changed(unit_idx: int, old_hp: int, new_hp: int, delta: int):
	...
	if unit_idx == 1 and delta < 0:
		...
		_done = true
		print("\n[%s] Formula 55 (PA*Y): expected=%d actual=%d  (HP: %d->%d)" % [
...
func on_victory(winning_team: int):
	if not _done:
		print("\n[FAIL] Formula 55: no damage observed before victory")
