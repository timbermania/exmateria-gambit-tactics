extends Node
## HEADFUL acceptance guard: "Add tween here" splits a SCREEN span at the STORABLE boundary
## NEAREST the right-clicked frame (ADR-0087 + ColorLowering.split_durations) — the screen
## mirror of EffectPaletteInsertPositionTest (the "it doesn't split where I right click"
## regression class; screen shares the chooser by construction, this guards the threading).
## Drives the real page path (_lane_context_actions → _run_lane_verb) on real E015:
...
## Run: <GODOT> --path . --quit-after 400 res://tests/EffectScreenInsertPositionTest.tscn
...
func _ready() -> void:
	await _run()
	print("\n=== EffectScreenInsertPositionTest: %d passed, %d failed ===" % [_passed, _failed])
	if _failed > 0:
		print("[FAIL] EffectScreenInsertPositionTest")
		get_tree().quit(1)
	else:
		print("[PASS] EffectScreenInsertPositionTest")
		get_tree().quit(0)
...
func _run() -> void:
	...
	for d in page._effect_dirs:
		if String(d).ends_with("E015"):
			dir = d
	if dir == "":
		print("[SKIP] E015 extract absent")
		return
	page._load_effect(dir)
	await _frames(20)

	var spans := _roomy_screen_spans(page, 3)
	if spans.is_empty():
		print("[SKIP] no roomy screen span on E015")
		return
...
func _probe_split(scn, page, span: Dictionary, frac: float) -> void:
	...
	var label := "screen/%s kf%d [%d,%d) click@%d" % [phase, kf_index, s, e, click_abs]
	...
	# The new (identity no-op) keyframe is kf_index+1; its span start on the rebuilt score is the cut.
	var new_id := "screen:%s#%d" % [phase, kf_index + 1]
...
func _probe_short_split(scn, page, span: Dictionary) -> void:
	...
	var label := "SHORT screen/%s kf%d [%d,%d) click@%d" % [phase, kf_index, s, e, click_abs]
...
func _roomy_screen_spans(page, n: int) -> Array:
	var out: Array = []
